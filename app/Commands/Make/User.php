...
class User extends Command
{
     ...
     * @var string
     */
    protected $description = 'Create a user class and table';
    ...
    public function handle()
    {
        ...
        $continue = true;
        $fieldNames = ["uuid", "username", "password", "role"];
        $types = ["boolean", "char", "date", "dateTime", "float", "integer", "longText", "tinyInteger", "string", "text"];
        $index = 3;

        if (
            file_exists($this->getEntityPath() . DIRECTORY_SEPARATOR . $entityName . ".php") ||
            file_exists($this->getRepositoryPath() . DIRECTORY_SEPARATOR . $entityName . "Repository.php") ||
            file_exists($this->getMigrationPath() . DIRECTORY_SEPARATOR . $entityName . "Migration.php")
        ) {
            $this->line("<error>Error !</error> This entity already exists");
        } else {

            $this->info("The following fields will be automatically added to this class : uuid, username, password and roles");


            $keepAdding = strtolower($this->ask("Add a new field ? (y/n)"));
            while (!in_array($keepAdding, ["y", "n", "yes", "no"])) {
            ...
            while ($continue) {
                ...
                while (in_array($fieldName, $fieldNames) || empty($fieldName)) {
                    if (in_array($fieldName, $fieldNames)) {
                        $this->error("This field already exists");
                    }

                    if (empty($fieldName)) {
                        $this->error("Empty field are invalid");
                    }
                ...
                //! field type
                $type = $this->ask("What's the type of the field");
